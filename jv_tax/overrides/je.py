...
class CustomJournalEntry(JournalEntry):
    ...
    def apply_custom_taxes(self):
        ...
        # ── Step 1: Snapshot accounting dimensions from existing tax rows ──
        # Keyed by account_head so they can be reapplied after rebuild.
        saved_dimensions = {}
        for row in self.accounts:
            if row.custom_is_tax_row:
                saved_dimensions[row.account] = {
                    "branch": row.branch or "",
                    "cost_center": row.cost_center or "",
                    "project": row.project or "",
                    # Add other accounting dimension fields here, e.g.:
                    # "department": row.department or "",
                }

        # ── Step 2: Remove previously injected tax rows ───────────────────
        self.accounts = [row for row in self.accounts if not row.custom_is_tax_row]

        # ── Step 3: Identify base rows ────────────────────────────────────
        base_rows = [row for row in self.accounts if row.custom_tax_included]
        ...
        for row in base_rows:
            # Use gross amount as stable base — never current debit/credit
            # (which may already be reduced from a prior calculation)
            gross = flt(row.custom_gross_amount)
            if not gross:
                gross = flt(row.debit_in_account_currency) or flt(
                    row.credit_in_account_currency
            ...
            net = flt(gross / (1 + total_rate / 100), 2) if total_rate else gross
            ...
            for i, tax in enumerate(valid_tax_rows):
                ...
                if i == len(valid_tax_rows) - 1:
                    # Last tax row absorbs any rounding remainder
                    tax_amount = flt(gross - net - row_tax_total, 2)
                else:
                    tax_amount = flt(net * rate / 100, 2)
                ...
                # Restore previously saved dimensions for this account (if any)
                dims = saved_dimensions.get(tax["account_head"], {})
                ...
                new_rows.append(
                    {
                        ...
                        # ── Restored accounting dimensions ──────────────────
                        "branch": dims.get("branch", ""),
            ...
            # ── Reduce base row to net (gross − tax) ─────────────────────
            if flt(row.debit_in_account_currency) or flt(row.debit):
                row.debit_in_account_currency = net
                row.debit = net
            else:
        ...
        # ── Step 4: Append rebuilt tax rows ──────────────────────────────
        for row_data in new_rows:
            self.append("accounts", row_data)
    ...
    def _restore_gross_amounts(self):
        """Restore base rows to their gross amount when template is removed."""
        for row in self.accounts:
            if row.custom_tax_included and flt(row.custom_gross_amount):
                gross = flt(row.custom_gross_amount)
                if flt(row.debit_in_account_currency) or flt(row.debit):
